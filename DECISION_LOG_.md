...
CREATE TABLE monthly_metrics (
    ...
    currency     TEXT NOT NULL,         -- 'USD' или 'multi' если /skip
...
CREATE TABLE anomaly_events (
    ...
    baseline_val REAL NOT NULL,         -- среднее за 3 предыдущих месяца
...
**Решение: synchronous hook в API router**

```python
# api/routers/ingest.py (после успешного ETL)
@router.post("/ingest/csv")
async def ingest_csv(...):
    result = await etl_pipeline.run(file)          # существующий ETL
    await metrics_service.recalculate(result.month_key)  # новый хук
    await anomaly_service.scan(result.month_key)   # детекция аномалий
    return result
```

**Порядок выполнения:**
1. ETL pipeline (существующий) — парсинг и загрузка в `transactions`
2. `metrics_service.recalculate(month_key)` — upsert в `monthly_metrics` + `category_metrics`
3. `anomaly_service.scan(month_key)` — детекция спайков, запись в `anomaly_events`
4. Ответ клиенту — включает `anomaly_count` если аномалии найдены

**Обработка ошибок:**
- Шаги 2-3 не блокируют ответ ETL при падении (try/except с логированием)
- ETL результат возвращается пользователю в любом случае
- Ошибки метрик логируются как WARNING, не CRITICAL

**Новое поле в ingest response:**
```json
{
  "transactions_loaded": 142,
  "month_key": "2026-03",
  "anomaly_count": 2,      // 0 если нет, null если insufficient_history
  "metrics_updated": true
}
```

**Вывод:**
Hook синхронный — простота важнее параллелизма на этом масштабе.
Если recalculate займёт > 2с на реальных данных — вынести в background task (Phase 3).
